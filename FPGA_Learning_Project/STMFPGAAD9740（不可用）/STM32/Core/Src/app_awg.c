...
/* Custom waveform data buffer */
static uint16_t g_custom_wave_data[2048];

/* Waveform parameters */
static uint8_t  g_duty_cycle     = 50;
static uint8_t  g_sinc_lobes     = 3;
static uint32_t g_noise_seed     = 12345;
static uint32_t g_exp_tau        = 200;
static uint32_t g_gaussian_sigma = 100;
...
static void Cmd_SetDuty(uint8_t duty);
static void Cmd_SetSincLobes(uint8_t lobes);
static void Cmd_SetNoiseSeed(uint32_t seed);
static void Cmd_SetExpTau(uint32_t tau);
static void Cmd_SetGaussianSigma(uint32_t sigma);
static void Cmd_LoadCustom(char *args);
...
static void LoadCurrentWaveform(void)
{
    ...
    switch (g_waveform) {
        ...
        case WAVE_SINC:         offset = 0;              gen_amplitude = 450; break;
        case WAVE_ARBITRARY:    offset = 0;              gen_amplitude = 0;   break;
        case WAVE_REV_SAWTOOTH: offset = DAC_MID_VALUE;  gen_amplitude = 511; break;
        case WAVE_EXP_DECAY:    offset = 0;              gen_amplitude = 450; break;
        case WAVE_GAUSSIAN:     offset = 0;              gen_amplitude = 450; break;
    ...
    switch (g_waveform) {
        ...
        case WAVE_TRIANGLE:
            ...
            break;
        case WAVE_SQUARE:
            Gen_SquareWave(wave_buf, WAVEFORM_POINTS, gen_amplitude, offset, g_duty_cycle);
            break;
        case WAVE_SAWTOOTH:
            Gen_SawtoothWave(wave_buf, WAVEFORM_POINTS, gen_amplitude, offset);
            break;
        case WAVE_NOISE:
            Gen_NoiseWave(wave_buf, WAVEFORM_POINTS, gen_amplitude, offset, g_noise_seed);
            break;
        case WAVE_DC:
            Gen_DCLevel(wave_buf, WAVEFORM_POINTS, gen_amplitude);
            break;
        case WAVE_SINC:
            Gen_SincWave(wave_buf, WAVEFORM_POINTS, gen_amplitude, g_sinc_lobes);
            break;
        case WAVE_ARBITRARY:
            Gen_CustomWave(wave_buf, WAVEFORM_POINTS, g_custom_wave_data, 2048);
            break;
        case WAVE_REV_SAWTOOTH:
            Gen_RevSawtoothWave(wave_buf, WAVEFORM_POINTS, gen_amplitude, offset);
            break;
        case WAVE_EXP_DECAY:
            Gen_ExpDecay(wave_buf, WAVEFORM_POINTS, gen_amplitude, g_exp_tau);
            break;
        case WAVE_GAUSSIAN:
            Gen_GaussianPulse(wave_buf, WAVEFORM_POINTS, gen_amplitude, g_gaussian_sigma);
            break;
        default:
            return;
    }
...
/* ============================================================
 * Command: Set Duty Cycle
 * ============================================================ */
static void Cmd_SetDuty(uint8_t duty)
{
    if (duty > 100) duty = 100;
    g_duty_cycle = duty;
    if (g_waveform == WAVE_SQUARE) LoadCurrentWaveform();
    UART_Console_Printf("OK: Duty cycle = %u%%\r\n", duty);
}

/* ============================================================
 * Command: Set Sinc Lobes
 * ============================================================ */
static void Cmd_SetSincLobes(uint8_t lobes)
{
    if (lobes < 1 || lobes > 10) {
        UART_Console_Printf("ERR: Lobes 1-10\r\n");
        return;
    }
    g_sinc_lobes = lobes;
    if (g_waveform == WAVE_SINC) LoadCurrentWaveform();
    UART_Console_Printf("OK: Sinc lobes = %u\r\n", lobes);
}

/* ============================================================
 * Command: Set Noise Seed
 * ============================================================ */
static void Cmd_SetNoiseSeed(uint32_t seed)
{
    g_noise_seed = seed;
    if (g_waveform == WAVE_NOISE) LoadCurrentWaveform();
    UART_Console_Printf("OK: Noise seed = %lu\r\n", seed);
}

/* ============================================================
 * Command: Set Exponential Decay Tau
 * ============================================================ */
static void Cmd_SetExpTau(uint32_t tau)
{
    if (tau < 1) tau = 1;
    g_exp_tau = tau;
    if (g_waveform == WAVE_EXP_DECAY) LoadCurrentWaveform();
    UART_Console_Printf("OK: ExpDecay tau = %lu samples\r\n", tau);
}

/* ============================================================
 * Command: Set Gaussian Sigma
 * ============================================================ */
static void Cmd_SetGaussianSigma(uint32_t sigma)
{
    if (sigma < 1) sigma = 1;
    g_gaussian_sigma = sigma;
    if (g_waveform == WAVE_GAUSSIAN) LoadCurrentWaveform();
    UART_Console_Printf("OK: Gaussian sigma = %lu samples\r\n", sigma);
}

/* ============================================================
 * Command: Load Custom Waveform Point
 *   LOAD <idx> <val>   Set single point
 * ============================================================ */
static void Cmd_LoadCustom(char *args)
{
    char *a1 = strtok(args, " ");
    char *a2 = strtok(NULL, " ");
    if (a1 && a2) {
        uint16_t idx = (uint16_t)atoi(a1);
        uint16_t val = (uint16_t)atoi(a2);
        if (idx < 2048 && val <= 1023) {
            g_custom_wave_data[idx] = val;
            UART_Console_Printf("OK: Custom[%u] = %u\r\n", idx, val);
            return;
        }
    }
    UART_Console_Printf("Usage: LOAD <idx> <val>  (idx:0-2047, val:0-1023)\r\n");
}
...
static void Cmd_SetWaveform(uint8_t wave)
{
    if (wave > WAVE_TYPE_MAX) {
        UART_Console_Printf("ERR: Waveform type 0-%d\r\n", WAVE_TYPE_MAX);
        return;
    }
    g_waveform = wave;
    LoadCurrentWaveform();
    FPGA_WriteReg(REG_WAVEFORM_BASE, 0);
    const char *names[] = {"Sine", "Triangle", "Square", "Sawtooth", "Noise",
                           "DC", "Sinc", "Arbitrary", "RevSawtooth", "ExpDecay", "Gaussian"};
...
static void Cmd_Status(void)
{
    const char *wave_names[] = {"Sine", "Triangle", "Square", "Sawtooth", "Noise",
                                "DC", "Sinc", "Arbitrary", "RevSawtooth", "ExpDecay", "Gaussian"};
...
static void UART_ProcessCommand(char *cmd)
{
    ...
    if (strcmp(tok, "FREQ") == 0) {
        ...
        else UART_Console_Printf("Usage: WAVE <0-10>\r\n");
    ...
    } else if (strcmp(tok, "DUTY") == 0) {
        char *arg = strtok(NULL, " ");
        if (arg) Cmd_SetDuty((uint8_t)atoi(arg));
        else UART_Console_Printf("Usage: DUTY <0-100>\r\n");

    } else if (strcmp(tok, "LOBES") == 0) {
        char *arg = strtok(NULL, " ");
        if (arg) Cmd_SetSincLobes((uint8_t)atoi(arg));
        else UART_Console_Printf("Usage: LOBES <1-10>\r\n");

    } else if (strcmp(tok, "SEED") == 0) {
        char *arg = strtok(NULL, " ");
        if (arg) Cmd_SetNoiseSeed((uint32_t)atol(arg));
        else UART_Console_Printf("Usage: SEED <0-4294967295>\r\n");

    } else if (strcmp(tok, "TAU") == 0) {
        char *arg = strtok(NULL, " ");
        if (arg) Cmd_SetExpTau((uint32_t)atol(arg));
        else UART_Console_Printf("Usage: TAU <samples>\r\n");

    } else if (strcmp(tok, "SIGMA") == 0) {
        char *arg = strtok(NULL, " ");
        if (arg) Cmd_SetGaussianSigma((uint32_t)atol(arg));
        else UART_Console_Printf("Usage: SIGMA <samples>\r\n");

    } else if (strcmp(tok, "LOAD") == 0) {
        char *rest = strtok(NULL, "");
        if (rest) Cmd_LoadCustom(rest);
        else UART_Console_Printf("Usage: LOAD <idx> <val>\r\n");

    } else if (strcmp(tok, "HELP") == 0) {
        UART_Console_Printf("\r\n=== Commands ===\r\n");
        UART_Console_Printf("FREQ <Hz>        Set frequency (1-62500000)\r\n");
        UART_Console_Printf("AMP <0-1023>     Set amplitude\r\n");
        UART_Console_Printf("WAVE <0-10>      Select waveform:\r\n");
        UART_Console_Printf("                 0=sine 1=tri 2=sqr 3=saw 4=noise\r\n");
        UART_Console_Printf("                 5=DC 6=sinc 7=arb 8=revsaw\r\n");
        UART_Console_Printf("                 9=expdecay 10=gaussian\r\n");
        ...
        UART_Console_Printf("DUTY <0-100>     Square wave duty cycle\r\n");
        UART_Console_Printf("LOBES <1-10>     Sinc side lobes\r\n");
        UART_Console_Printf("SEED <n>         Noise RNG seed\r\n");
        UART_Console_Printf("TAU <samples>    ExpDecay time constant\r\n");
        UART_Console_Printf("SIGMA <samples>  Gaussian sigma\r\n");
        UART_Console_Printf("LOAD <i> <v>     Set custom waveform point\r\n");
        UART_Console_Printf("================\r\n");
...
void APP_AWG_Init(void)
{
    ...
    /* Initialize custom waveform buffer with a test pattern */
    for (int i = 0; i < 2048; i++) {
        float phase = 2.0f * (float)M_PI * (float)i / 2048.0f;
        g_custom_wave_data[i] = (uint16_t)(DAC_MID_VALUE +
            (int32_t)(511.0f * sinf(phase) + 0.5f));
    }
