...
//		printf("CTP ID:%x\r\n",((u16)temp[0]<<8)+temp[1]);
    ...
    {
        printf("can not regnize the vertion of 7016");
    }
	return res;
}
...
u8 FT5206_Scan(u8 mode)
{
	...
	t++;
//	if(t%2==0|t<2)//空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率
//	{
		...
		if((mode&0XF)&&((mode&0XF)<=g_gt_tnum))
		{
			...
			for(i=0;i<g_gt_tnum;i++)
			{
				if(tp_dev.sta&(1<<i))		//触摸有效?
                {
                    ...
                    {
                        tp_dev.x[i]=PIXELS_W-(((u16)(buf[0]&0X0F)<<8)+buf[1]);
                        tp_dev.y[i]=((u16)(buf[2]&0X0F)<<8)+buf[3];
                    }
				}			
			} 
			res=1;
            if(tp_dev.x[0]>PIXELS_W||tp_dev.y[0]>PIXELS_H)  //非法数据(坐标超出了)
            {
